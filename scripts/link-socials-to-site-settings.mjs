...
async function run() {
  console.log("Linking socials to site settings...\n");
  const space = await client.getSpace(SPACE_ID);
  const env = await space.getEnvironment(ENV_ID);

  try {
    // Get all social entries
    const socials = await env.getEntries({
      content_type: "social",
      limit: 1000,
    });

    console.log(`Found ${socials.items.length} social entries`);

    if (socials.items.length === 0) {
      console.log("⚠ No social entries found. Run contentful:seed first to create socials.");
      return;
    }
    ...
    if (siteSettings.items.length === 0) {
      console.log("⚠ No site settings found. Run contentful:upsert-site-settings first.");
      return;
    }
    ...
    if (site.sys.publishedVersion) {
      ...
      // Update socials field with new entry IDs
      freshSite.fields.socials = {
        "en-US": socials.items.map((entry) => ({
    ...
    console.log(`✓ Updated site settings with ${socials.items.length} social references`);
    console.log("\n✔ Socials linking complete!");
  } catch (e) {
